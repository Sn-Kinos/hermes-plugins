...
새 문구는 `tpl()`로 추가합니다. 업스트림 원문을 **그대로** 적고 `{}` 자리에 보간이 들어간다고
표시하면 앵커된 정규식이 만들어지므로, 손으로 정규식을 쓰다 틀릴 일이 없고 소스 줄과 1:1로
대조됩니다. 한국어 쪽에서 `{0}` `{1}`로 순서를 바꿔 쓸 수 있습니다.
...
| `SEND_REWRITES` | 고정 시스템 문구 (제공자 오류·재시도 포함) |
